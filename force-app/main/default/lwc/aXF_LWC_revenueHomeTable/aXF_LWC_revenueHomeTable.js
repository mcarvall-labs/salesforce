...
const STATUS_META = {
  ...
  RECEBIDA: {
    ...
    label: AXF_LBL_StatusReceived
  },
  RECEBIDA_EM_ATRASO: {
    icon: "utility:like",
    variant: "slds-icon-text-success",
    label: AXF_LBL_StatusReceivedLate
  },
  VENCIDA: {
    icon: "utility:dislike",
...
export default class AXF_LWC_revenueHomeTable extends LightningElement {
  ...
  decorateRow(row) {
    ...
    return {
      ...
      canSettle:
        row.AXF_CF_PKL_Status__c === "PENDENTE" ||
        row.AXF_CF_PKL_Status__c === "VENCIDA"
    };
  }
  ...
  async handleSave() {
    await this.settle(
      this.selectedTransaction && this.selectedTransaction.type === "bank"
        ? this.selectedTransaction.id
        : null,
      this.selectedTransaction && this.selectedTransaction.type === "card"
        ? this.selectedTransaction.id
        : null
    );
  }

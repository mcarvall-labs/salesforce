...
export default class AXF_LWC_quickFlowActions extends LightningElement {
  ...
  get showFinancingFields() {
    return ["FINANCIAMENTO", "FINANCIAMENTO_SAC"].includes(
  ...
  get isPastDueOpen() {
    return (
      ...
      this.form.firstDueDate < new Date().toISOString().substring(0, 10)
    );
  }
  ...
  get bankAccountFilter() {
    return {
      criteria: [
        {
          fieldPath: "AXF_BA_LKP_Account__c",
          operator: "eq",
          value: this.form.accountId || null
        }
      ]
    };
  }
  ...
  get creditCardFilter() {
    return {
      criteria: [
        {
          fieldPath: "AXF_CC_LKP_Account__c",
          operator: "eq",
          value: this.form.accountId || null
        }
      ]
  ...
  get amountLabel() {
    return this.form.valueType === "TOTAL"
      ? "Valor Total (R$)"
      : "Valor da Parcela (R$)";
  }
  ...
  handleInputChange(event) {
    ...
    if (name === "recurringType") {
      this.form.provisioningStrategy =
        this.form.recurringType === "VARIAVEL" ? "MEDIA_MOVEL" : "";
    }
  ...
  resetModalityFields() {
    ...
    this.form.financingType =
      this.form.paymentMode === "FINANCIAMENTO_SAC" ? "SAC" : "";
    this.form.monthlyAdjustmentRate = null;
    ...
    try {
      ...
      if (this.isPastDueOpen) {
        const confirmed = await LightningConfirm.open({
          ...
          message:
            "Este lançamento continuará em aberto e será criado com status Vencida. Deseja continuar?",
          variant: "headerless"
        });
        if (!confirmed) {
          return;
      ...
      this.isLoading = true;
      await createCashFlowSeries({ dto: this.buildDto() });
      this.showToast(
        "Sucesso",
        this.modalTitle + " registrada com sucesso!",
        "success"
      );
      this.isModalOpen = false;
      this.dispatchEvent(
  ...
  buildDto() {
    return {
      ...
      totalInstallments: this.showInstallmentCount
        ? Number(this.form.totalInstallments) || 1
        : null,
      ...
      provisioningStrategy: this.showProvisioningStrategy
        ? this.form.provisioningStrategy
        : null,
      financingType: this.showFinancingFields ? this.form.financingType : null,
      monthlyAdjustmentRate: this.showSacRate
        ? this.toOptionalNumber(this.form.monthlyAdjustmentRate)
        : null,
      adjustmentAnniversary: this.showConsortiumFields
        ? this.form.adjustmentAnniversary
        : null,

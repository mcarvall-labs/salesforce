...
export default class AXF_LWC_expenseHomeTable extends LightningElement {
    labels = { title: AXF_LBL_ExpensesHomeTitle, dueDate: AXF_LBL_DueDate, value: AXF_LBL_ColumnValue,
        status: AXF_LBL_Status, reconciled: AXF_LBL_ColumnReconciled, modalTitle: AXF_LBL_SettleExpenseModalTitle,
        paymentDate: AXF_LBL_PaymentDate, reconcileWithBank: AXF_LBL_ReconcileWithBankTransaction,
        reconcileWithCard: AXF_LBL_ReconcileWithCardTransaction, settleInCash: AXF_LBL_SettleInCash,
        save: AXF_LBL_ModalSave, cancel: AXF_LBL_ModalCancel, noMovementsFound: AXF_LBL_NoMovementsFound };
    currencyCode = ORG_CURRENCY_CODE; isLoading = true; rows = []; wiredResult;
    selectedCategoryId = ''; categoryRecordType = CATEGORY_RECORD_TYPE; isModalOpen = false;
    selectedRowId; paymentDate = this.today(); paidValue; bankMatches = []; cardMatches = []; selectedTransaction;
    handleCategoryFilterChange(event) { this.selectedCategoryId = event.detail.value; }
    @wire(getMonthlyMovements, { recordTypeDeveloperName: RECORD_TYPE, categoryId: '$selectedCategoryId' })
    wiredMovements(result) { this.wiredResult = result; this.isLoading = false; if (result.data) this.rows = result.data.map((row) => this.decorateRow(row)); }
    get hasRows() { return this.rows && this.rows.length > 0; }
    get hasNoRows() { return !this.isLoading && !this.hasRows; }
    get hasNoSelection() { return !this.selectedTransaction; }
    get transactionMatches() { return [...this.bankMatches, ...this.cardMatches]; }
    get hasMatches() { return this.transactionMatches.length > 0; }
    decorateRow(row) {
    ...
    async handleSettleClick(event) {
        this.selectedRowId = event.currentTarget.dataset.id; this.paymentDate = this.today();
        const row = this.rows.find((item) => item.id === this.selectedRowId); this.paidValue = row ? row.value : null;
        this.bankMatches = []; this.cardMatches = []; this.selectedTransaction = undefined; this.isModalOpen = true;
        try {
            const values = await getReconciliationSuggestions({ cashFlowId: this.selectedRowId });
            const mapped = values.map((item) => ({ id: item.transactionId, type: item.source === 'BANK' ? 'bank' : 'card',
                amount: Math.abs(item.amount), transactionDate: item.transactionDate,
                label: `${item.confidence} · ${item.description || '-'} · ${item.transactionDate} · ${Math.abs(item.amount)}` }));
            this.bankMatches = mapped.filter((item) => item.type === 'bank'); this.cardMatches = mapped.filter((item) => item.type === 'card');
        } catch (error) { this.showErrorToast(error); }
    }
    handleCloseModal() { this.isModalOpen = false; }
    handlePaymentDateChange(event) { this.paymentDate = event.detail.value; }
    handlePaidValueChange(event) { this.paidValue = event.detail.value; }
    handleSelectTransaction(event) {
        const id = event.currentTarget.dataset.id; const type = event.currentTarget.dataset.type;
        const match = this.transactionMatches.find((item) => item.id === id); this.selectedTransaction = { id, type };
        if (match) { this.paidValue = match.amount; this.paymentDate = match.transactionDate; }
    }
    async handleUnreconcile(event) { try { await unreconcile({ cashFlowId: event.currentTarget.dataset.id }); await refreshApex(this.wiredResult); } catch (error) { this.showErrorToast(error); } }
    async handleSave() { await this.settle(this.selectedTransaction?.type === 'bank' ? this.selectedTransaction.id : null, this.selectedTransaction?.type === 'card' ? this.selectedTransaction.id : null); }
    async handleSettleInCash() { await this.settle(null, null); }
    ...
    showErrorToast(error) { const detail = error?.body?.message || error?.message; this.dispatchEvent(new ShowToastEvent({ variant: 'error', title: 'Erro', message: detail || 'Erro desconhecido' })); }
}

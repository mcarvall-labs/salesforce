...
export default class AXF_LWC_investmentCapacityKpi extends NavigationMixin(LightningElement) {
    @track metrics = {
    ...
    loadMetrics() {
        return getInvestmentCapacityMetrics({ selectedAccountId: this.selectedAccountId })
            .then((data) => {
                this.metrics = data;
            })
            .catch((error) => {
                const errorMsg = (error && error.body && error.body.message) || error.message || 'Erro ao carregar métricas.';
                this.showToast('Erro', errorMsg, 'error');
            });
    }

    get hasPositiveAccounts() {
        return this.metrics && this.metrics.positiveBankAccounts && this.metrics.positiveBankAccounts.length > 0;
    }
    ...
    get geralTabItemClass() {
        return this.isGeneralTab ? 'slds-tabs_default__item slds-is-active' : 'slds-tabs_default__item';
    }
        ...
        triggerSyncAllConnections()
            .then((message) => {
                return this.loadMetrics().then(() => {
                    this.showToast('Sucesso', message || 'Atualização iniciada com sucesso.', 'success');
                });
            })
            .catch((error) => {
                const errorMsg = (error && error.body && error.body.message) || error.message || 'Erro ao processar atualização.';
                this.showToast('Erro', errorMsg, 'error');
            })
            .finally(() => {

...
| Webhook                             | `AXF_CLS_PluggyWebhookResource` (Site guest user) — HMAC-SHA256 **constant-time** signature check **before** anything is accepted; a valid payload only **publishes `AXF_PluggyWebhookEvent__e`** (guest can only `Create`), never stored as a fact; secret in a protected Custom Metadata type (`AXF_PluggyWebhookConfig__mdt`). `AXF_TRG_PluggyWebhookEvent` → `AXF_CLS_PluggyWebhookEventHandler` runs the consent revalidation **out of the guest context** (Automated Process user) via the bulk `AXF_CLS_PluggyConsentService.refreshByItemIds` (all callouts before the single DML) |
...
- **67 Apex tests + Jest suite — all green.** Per-class Apex coverage 78–100% on the
...
4. **Webhook → platform event bridge.** The Salesforce Site guest-user license forbids
   ...
   the consent-revalidation callout+DML itself. It verifies the HMAC and publishes
